...
var Upload = {
    ...
    upload: function () {
        ...
        this.resourceDom = this.wrapperDom.find('#upload-resource'),

            this.outputDom = this.wrapperDom.find('#upload-output'),

            this.progressBarDom = this.wrapperDom.find('#upload-progressbar'),
...
function upload(resource) {
    ...
    var newInstance = Object.create(Upload);

    newInstance.wrapperDom = $(resource).parents('#upload-wrapper');

    newInstance.group = 'file'; //分组的默认值

    newInstance.savedPathDom = newInstance.wrapperDom.find('#upload-savedpath'); //资源储存地址所在节点的默认值
